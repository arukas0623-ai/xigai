...
window.XIGAI["人工智能"] = [
  ...
  },
  {
    ...
    "principle": "强化学习的核心机制在于智能体通过与环境的交互来学习最优的行为策略。在每次交互中，智能体根据当前状态采取动作，并接收来自环境的奖励或惩罚信号作为反馈。这些反馈帮助智能体调整其行为策略，以期在未来获得更高的累积回报。这一过程可以通过多种算法实现，如Q-learning、深度Q网络（DQN）、策略梯度与近端策略优化（PPO）等。强化学习通过不断试错和适应，逐步逼近最优解。",
    "pros": [
      "能够处理具有长期依赖性的任务",
      "适用于探索未知环境的场景",
      "可以实现自我改进和优化"
  ...
  },
  {
    ...
    "principle": "卷积神经网络的原理在于通过卷积层使用卷积核在输入数据上滑动，提取局部特征；激活层引入非线性变换增强模型表达能力；池化层通过最大值或平均值下采样降低分辨率并提高鲁棒性；全连接层将高层抽象特征映射到输出类别。这种结构使得CNN能够自动从低级特征（如边缘、纹理）逐步学习到高级特征（如物体部件），从而实现图像分类等任务。",
    "pros": [
      "减少参数量，避免过拟合",
      "提取平移不变的局部特征",
      "适用于网格结构数据"
  ...
  },
  {
    ...
    "principle": "注意力机制通过计算查询（Query）与键（Key）之间的相似度得分，利用Softmax函数将这些得分转换为权重，并对值（Value）进行加权求和，从而动态地聚焦于当前任务最重要的信息。这一过程允许模型在处理序列或图像时，根据需要关注不同的部分，提高了处理效率和准确性。",
    "pros": [
      "能够解决长距离依赖问题",
      "提升模型的并行计算能力",
      "增强对全局信息的理解"
